...
- **Rendering stack (pick one)**:  
  DOM/Components (Vanilla, Web Components, Lit, React, Vue) **or** Canvas/WebGL (Phaser, Canvas, PixiJS, p5, Three.js) **or** Viz/Maps (D3, Plotly, Chart.js, ECharts, Leaflet, MapLibre). **You own orchestration**: state → render, lifecycles, effects (not just helper calls). :contentReference[oaicite:1]{index=1}
- **Architecture**: single HTML shell with router/view-manager; **≥ 6 ES modules** and **≥ 3 meaningful classes** (model, system/controller, renderer/view). :contentReference[oaicite:2]{index=2}
- **Networking**: at least one **public GET** (open API / published CSV/JSON) with loading/empty/error states + TTL cache; at least one **cloud write (PUT/POST)** of a non-sensitive artifact (e.g., preset/run/leaderboard row). :contentReference[oaicite:3]{index=3}
- **Local-first**: autosave and restore from `localStorage`/IndexedDB; app boots from local state before network. (Export/Import JSON recommended.) :contentReference[oaicite:4]{index=4}
...
- **Sprint 3 (Days 11–21): Full + Polish** — complete features, interaction depth, responsiveness, docs + deploy. :contentReference[oaicite:5]{index=5}
...
- Clear responsibilities (model • system/controller • renderer/view) :contentReference[oaicite:6]{index=6}
...
- Resize gracefully for laptop (≥1280×720) and tablet (≥768px). If constrained (e.g., keyboard + landscape), show a **Requirements/Not-supported** screen. :contentReference[oaicite:7]{index=7}
...
- Restore on launch from local storage, then merge network changes :contentReference[oaicite:8]{index=8}
...
**Deploy**: GitHub Pages (use **hash routing** or a 404 redirect so deep links work). Provide a local run fallback (e.g., `npm run dev` or `npx http-server .`). :contentReference[oaicite:9]{index=9}
...
- Project board with **8–12** Sprint-2 issues (title, brief AC, labels, size, assignee); board link in README :contentReference[oaicite:10]{index=10}

**Sprint 2 — MVP (Days 3–10)**  
- From a clean clone, app runs; vertical slice: **input → state → render → GET → PUT/POST**  
- UI states for network; console clean; `/src/` has **≥6 modules** & **≥3 classes**; 30–60s GIF of the flow :contentReference[oaicite:11]{index=11}
...
- Final README (product-facing) + dev docs; GH Pages deploy or bullet-proof local run; release tag `v1.0-full` :contentReference[oaicite:12]{index=12}
...
- **Credits/Licenses**; **Developer docs links** (`/docs/...`) :contentReference[oaicite:13]{index=13}
...
Bonus: Showcase-worthy submissions may receive extra credit. (See full rubric in the Module-02 PDF.) :contentReference[oaicite:14]{index=14}
...
- **Module-02 Project Spec** — full requirements, tracks, timeline, rubrics. :contentReference[oaicite:15]{index=15}
